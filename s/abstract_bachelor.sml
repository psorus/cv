...
These might not be very fast to train, but the group had nearly infinite computational resources for their other analysis tasks, so this was not a problem. A more interesting question was the type of machine learning task: Since charges of incoming nuclei are quantizied to integer mulplicates of electron charges, it seems to be appropriate to use a classification network, but since we prefer a particle of charge 9 to be identified as a particle of charge 8 than of a charge 2, using a regression task has also benefits.
I tried both to compare them later. Sadly this comparison is not so easy, since finding a clear loss function is not trivial (If you also take a look at my master thesis (or its [LI abstract_master:abstractNK]), there I write multiple subchapters on how I defined my loss function for a choice between 2 classes, and here I have 8 classes, that are also distributed drastically different: While basically all particles have a low charge, those particles with a high charge are usually the most interesting).
I preferred the regression task, since even though a regression network allows for basically every charge, by training it on quantizied outputs, you also get mostly quantizied outputs, and the deviation from this quantizied expectations is a good indication of the uncertainty of the classification (Even though it is not simply the uncertainty).
